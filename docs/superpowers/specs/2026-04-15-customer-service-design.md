...
- **EDA 发现**: `4k` 与 `20k` / `28k` **无重叠样本**; 但 `20k` 与 `28k` 之间存在 **8,306 条重复** (约 40% 重叠). 合并去重后的实际独立英德样本约 **~20,000+ 条**, 而非表面的 48,000 条. `version` 字段(`51` / `52` / `400`) 很可能是数据提供方的内部批次标记.
- **用途**: 用于 **Supervised 系统的英德双语增强训练**. 通过对比"仅使用 4k 全语言小数据"与"使用 ~20k 英德增强数据"的模型表现, 可以分析"为提升主流语言性能而扩大数据规模"是否本身会加剧对小语种的覆盖缺失, 这本身是一个重要的 Fairness & Bias 讨论点.
...
- **EDA 发现**: 该数据集的 **queue 分类体系与主数据集完全不同** (如 `Data Analytics & Reporting`, `Partner & Vendor Coordination`, `Release Management` 等), 因此**无法直接用于分类准确性对比**.
- **用途**: 仅用于 **纯文本层面的定性观察**——例如观察 Rule-Based 的关键词在标准化德语输入中的匹配情况, 以说明规则系统对非标准语言风格(拼写错误、口语化、缩写)的潜在脆弱性. 任何涉及该数据集的讨论均不纳入分类指标统计.
...
- **训练集**:
  ...
  - 增强训练数据(可选): `20k` / `28k` 数据集. **使用前必须做去重**:
    - `4k` 与 `20k` / `28k` 无重复, 可直接隔离;
    - `20k` 与 `28k` 之间有 **8,306 条重叠**, 合并使用时需通过 `subject` + `body` 去重, 实际独立样本约 ~20k+;
    - 同时移除任何与 `4k` 测试集/验证集重复的样本, 避免数据泄露.
    - 若对数据集间关系存疑, 保守策略是仅使用 `4k` 自身划分出的训练数据.
