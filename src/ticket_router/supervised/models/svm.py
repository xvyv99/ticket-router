...
from ticket_router.base.predictor import Trainer, Predictor, register_model
...
@register_model
class SVMPredictor(Predictor[SupervisedCfg]):
    name = "svm"
    dataset: BaseDataset
...
class SVMTrainer(Trainer):
    ...
    def train(
        ...
        for task in self.dataset.all_tasks:
            ...
            model = train_svm(texts, labels, f"svm_{task.name}", encoder=encoder)
            models[task.name] = model

        return SVMPredictor(dataset=self.dataset, models=models, cfg=self.cfg)
